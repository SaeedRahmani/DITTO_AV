#!/usr/bin/env python3
"""
Graph-DITTO Option 2: Multi-Agent Shared-Policy Training Pipeline.

Usage:
    python -m graph_ditto.train_option2 [--config path/to/option2_config.yaml]

Stage 1: Train the graph-based world model on expert driving data.
Stage 2: Train shared multi-agent policy via DITTO reward in the learned world model.
"""
...
from graph_ditto.trainers.multi_agent_trainer import MultiAgentTrainer
...
def main():
    parser = argparse.ArgumentParser(description="Graph-DITTO Option 2: Multi-Agent")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(os.path.dirname(__file__), "config", "option2_config.yaml"),
    ...
    if args.stage in ("wm", "both"):
        logger.info("=" * 60)
        logger.info("STAGE 1: World Model Training (Multi-Agent)")
        logger.info("=" * 60)
    ...
    # --- Stage 2: Multi-Agent Policy Training ---
    if args.stage in ("policy", "both"):
        ...
        logger.info("=" * 60)
        logger.info("STAGE 2: Multi-Agent Policy Training (DITTO)")
        logger.info("=" * 60)

        ma_trainer = MultiAgentTrainer(config, wm_checkpoint)
        ma_trainer.train()

        logger.info("Multi-agent policy training complete.")
